...
export default function AdminResultadosPage() {
  const t = useTranslations('AdminResultados')
  ...
  async function readPdfFile(file: File): Promise<PdfFile> {
    return new Promise((resolve, reject) => {
      if (file.type !== 'application/pdf') {
        reject(new Error(t('soloPdf')))
        return
      }
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return
    try {
      setPdfFile(await readPdfFile(file))
      setSaveError(null)
    } catch (err: unknown) {
  ...
  async function handleSave() {
    ...
    if (drawerMode === 'create' && !pdfFile) { setSaveError(t('debesSeleccionarPdf')); return }
    ...
    try {
      ...
      if (drawerMode === 'create') {
        ...
        await updateResult(selected!.id, data)
      }
      setDrawerOpen(false)
      load()
    } catch (err: unknown) {
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8 lg:py-12">
        ...
        {!loading && results.length === 0 && (
          <div className="text-center py-24">
            <PdfIcon className="w-10 h-10 mx-auto mb-3" style={{ color: 'var(--color-outline)' }} />
                  ...
                  <tr key={item.id}
                    ...
                    <td className="px-4 py-3 hidden lg:table-cell">
                      <span className="text-xs" style={{ color: 'var(--color-on-surface-variant)' }}>
                        {formatBytes(item.pdfOriginalSize)}
                      </span>
                    </td>
                    <td className="px-4 py-3">
          ...
          <aside className="w-full max-w-lg flex flex-col shadow-2xl overflow-y-auto"
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b"
              ...
              <h2 className="font-bold text-lg" style={{ color: 'var(--color-on-surface)', letterSpacing: '-0.01em' }}>
                {drawerMode === 'create' ? t('nuevoResultado') : t('editarResultado')}
              </h2>
            ...
            <div className="flex-1 px-6 py-6 space-y-5">
              ...
              <Field label={t('campoTitulo')}>
                <input
                  ...
                  placeholder={t('placeholderNombreResultado')}
                  maxLength={300}
                />
              </Field>
              ...
              {/* PDF upload */}
              <Field label={drawerMode === 'edit' ? t('campoReemplazarPdf') : t('campoArchivoPdf')}>
                <div
                  ...
                  {pdfFile ? (
                    <div className="flex items-center justify-center gap-3">
                      <PdfIcon className="w-8 h-8 flex-shrink-0" style={{ color: 'var(--color-primary-cta)' }} />
                      <div className="text-left">
                        ...
                        <p className="text-xs mt-0.5" style={{ color: 'var(--color-on-surface-variant)' }}>
                          {formatBytes(pdfFile.size)} · {t('hazClicParaCambiar')}
                        </p>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      {drawerMode === 'edit' && selected?.pdfFileName && (
                        <p className="text-xs mt-2 font-medium" style={{ color: 'var(--color-primary-cta)' }}>
                          {t('archivoActual', { name: selected.pdfFileName, size: formatBytes(selected.pdfOriginalSize) })}
                        </p>
                      )}
                    </>
                  )}
